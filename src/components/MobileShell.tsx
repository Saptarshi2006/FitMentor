  ...
  return (
    <div className="relative mx-auto flex min-h-screen w-full max-w-md flex-col bg-background">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 mx-auto w-full max-w-md px-4 pb-4">
        <div className="relative flex items-center justify-around rounded-3xl border border-white/10 bg-card/70 px-2 py-2 shadow-card backdrop-blur-2xl">
          {TABS.map((t) => {
            const Icon = t.icon;
            const active =
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active && !accent ? "text-foreground" : "text-muted-foreground hover:text-foreground",
                )}
              >
                {accent ? (
                  <span
